...
db.notes.insert({ title: 'Delete', content: 'Delete delete delete delete' });
...
db.notes.insertMany([{ title: 'Essential', content: 'Whatever dude' },{ title: 'Poodles', content: 'Apricot poodles' }]);
...
db.notes.update({ _id: '000000000000000000000003' }, { title: 'New title', content: 'New Content' });
...
db.notes.update({ _id: '000000000000000000000007' },{ $set: { title: 'New title 2.0' }});
...
db.notes.update({ _id: { $gt: '000000000000000000000014' }},{ $set: { title: 'All these titles', content: 'All this content' }},{ multi: true });
...
db.notes.find({ title: { $exists: false }});
...
db.notes.find({ $and: [{ title: { $not: /dog/i }}, { title: { $exists:true }}]}).sort({ _id: 1 }).pretty();